import pytest
import numpy as np
from surface_mesher.cuboid import generate_cuboid_surface, generate_axis_coords, generate_face_quads, Cuboid
...
def test_create_mesh_from_edge_sizes_invalid_shape():
    cuboid = Cuboid(length=2.0, width=1.0, height=1.0)
    with pytest.raises(ValueError, match="mesh_sizes must be a single float or an array of three floats."):
...
def test_create_mesh_from_edge_sizes_too_large():
    cuboid = Cuboid(length=2.0, width=1.0, height=1.0)
    with pytest.raises(ValueError, match="mesh_sizes must be less than or equal to the cuboid dimensions."):
...
def test_create_mesh_with_resolution_invalid_shape():
    cuboid = Cuboid(length=2.0, width=1.0, height=1.0)
    with pytest.raises(ValueError, match="divisions must be a single int or an array of three ints."):
